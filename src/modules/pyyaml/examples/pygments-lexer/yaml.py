...
class YAMLLexer(ExtendedRegexLexer):
    ...
    tokens = {
        ...
        "root": [
            ...
            (r"#[^\n]*", Comment.Single),
            # the '%YAML' directive
            (r"^%YAML(?=[ ]|$)", reset_indent(Name.Directive), "yaml-directive"),
            # the %TAG directive
            (r"^%TAG(?=[ ]|$)", reset_indent(Name.Directive), "tag-directive"),
            # document start and document end indicators
            (
                r"^(?:---|\.\.\.)(?=[ ]|$)",
                reset_indent(Punctuation.Document),
                "block-line",
            ),
            # indentation spaces
            (
        ...
        "block-scalar-content": [
            ...
            (
                r"^[ ]+$",
                parse_block_scalar_empty_line(Text.Indent, Literal.Scalar.Block),
            ),
