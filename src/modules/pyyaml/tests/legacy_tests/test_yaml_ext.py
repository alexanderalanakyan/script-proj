...
def test_c_scanner(data_filename, canonical_filename, verbose=False):
    ...
    with (
        open(canonical_filename, "rb") as file1,
        open(canonical_filename, "rb") as file2,
    ):
        _compare_scanners(file1, file2, verbose)
    with (
        open(canonical_filename, "rb") as file1,
...
import test_tokens, test_structure, test_errors, test_resolver, test_constructor, test_emitter, test_representer, test_recursive, test_input_output
