...
class PyYAMLItem(pytest.Item):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            os.path.basename(filenames[0]) if filenames else parent.name,
            parent,
            config,
            session,
            nodeid,
...
class PyYAMLCollector(pytest.Collector):
    ...
    def collect(self):
        ...
        if unittest is True:  # no filenames
            items.append(
                PyYAMLItem.from_parent(
                    parent=self, function=self._function, filenames=None
                )
            )
        else:
...
def pytest_pycollect_makeitem(collector, name: str, obj: object):
    ...
    if (
        unittest is True
    ):  # no file list to run against, just return a test item instead of a collector
        outcome.force_result(
            PyYAMLItem.from_parent(
                name=name, parent=collector, fspath=collector.fspath, function=obj
            )
        )
        return
...
def pytest_ignore_collect(collection_path: pathlib.Path):
    ...
    if basename == "test_yaml_ext.py":
        ...
        if require_libyaml == "1" and not HAS_LIBYAML_EXT:
            raise RuntimeError(
                "PYYAML_FORCE_LIBYAML envvar is set, but libyaml extension is not available"
            )
