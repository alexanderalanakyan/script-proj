...
def _filter_omit_entries(value):
    if isinstance(value, MutableMapping):
        ...
        return {
            k: v
            for k, v in ((k, _filter_omit_entries(v)) for k, v in value.items())
            if v is not ...
        }
...
def main():
    ...
    required_grp.add_argument(
        "--from-stdin", action="store_true", help="read input YAML from stdin"
    )
    required_grp.add_argument(
        "--from-file", type=pathlib.Path, help="read input YAML from file path"
    )
    ...
    if gh_output := os.environ.get("GITHUB_OUTPUT"):
        ...
        with pathlib.Path(gh_output).open("a") as env_fd:
            env_fd.write(
                f"matrix_json<<__MATRIX_EOF\n{output_matrix_json}\n__MATRIX_EOF\n"
            )
            env_fd.write(
                f"skipped_matrix_json<<__MATRIX_EOF\n{output_skipped_matrix_json}\n__MATRIX_EOF\n"
            )
    else:
