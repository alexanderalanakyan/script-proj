...
if main_cfg.get("dotfiles_enabled") == "True" and user_home:
    if Path.exists(user_home / ".dotfiles"):
        ...
        if src and not dest.exists() and not input(f"Input (y or n) to symlink: {src} to {dest}").lower().strip().startswith("n"):
            try:
...
if user_cfg.get("user_services"):
    f.run(["systemctl", "--user", "enable", "--now", *user_cfg.get("user_services", [])])
